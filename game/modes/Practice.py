...
class Practice(BaseGame):
    ...
    def end_game(self, context={}):
        ...
        self.players[0].add_server_message("You win!", is_json=False)
        self.players[0].add_server_message("You did that in " + str(time_difference) + " seconds", is_json=False)
        self.players[0].add_server_message("You did that in " + str(self.move_id) + " moves", is_json=False)
        self.players[0].add_server_message("Because this was practice, no iiq or currency change will happen.", is_json=False)
        self.players[0].add_server_message("[DONE_BUTTON]", is_json=False)
        self.players[0].add_server_message("[RELEASE]", is_json=False)
